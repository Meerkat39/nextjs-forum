...
### 8. 投稿編集・削除機能の実装
*   \[x\] **編集機能**:
    *   \[x\] `app/posts/[id]/edit/page.tsx` を作成し、編集フォームを設置する。
    *   \[x\] Server Actionで投稿の更新処理を実装する。
*   \[ \] **削除機能**:
    *   \[ \] 投稿詳細ページに削除ボタンを設置する。
...
### 9. Zodによる入力バリデーション
*   \[x\] `zod` をインストールする。
*   \[x\] 投稿作成・編集フォームの入力値に対するスキーマをZodで定義する。
*   \[x\] Server Action内でスキーマを使い、バリデーション処理を追加する。
*   \[x\] バリデーションエラーが発生した場合、エラーメッセージをフォームに表示する。

### 10. UIスタイリング (Tailwind CSS)
*   \[x\] アプリ全体のレイアウト (`layout.tsx`) に基本的なスタイルを適用する。
*   \[x\] ヘッダーやナビゲーションなどの共通コンポーネントを作成し、スタイルを適用する。
*   \[x\] 投稿一覧、投稿詳細、フォームなどの各ページ・コンポーネントのデザインをTailwind CSSで整える。

### 11. CI (継続的インテグレーション) の導入
*   \[x\] GitHub Actionsのワークフローファイル (`.github/workflows/ci.yml`) を作成する。
*   \[x\] 依存関係のインストール、Next.jsのビルド、リンティング、型チェックを行うステップを定義する。
